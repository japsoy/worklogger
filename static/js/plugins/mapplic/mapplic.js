...
(function($) {
	...
	var Mapplic = function() {
		...
		self.o = {
			...
			animate: true,
			maxscale: 4,
			level_changed: function(target) {},
			location_changed: function(id, x, y, el) {},
			pin_label_render: function(data) {
				return '';
			}
		};

		self.fetchAndShowData = function() {
			// Process JSON file
		...
		self.init = function(el, params) {
			...
			self.fetchAndShowData();

			return self;
		}

		self.getcoords = function(e) {
			var x = (e.pageX - self.map.offset().left) / self.map.width(), y = (e.pageY - self.map.offset().top) / self.map.height();
			return {x: x, y: y}
		}
		...
		function Tooltip() {
			...
			this.position = function(location) {
				//console.debug('TOOLTIP position:', location);
		...
		var processData = function(data) {
			...
			//$('div#map.mapplic-element').remove()
			self.el.html('');
			...
			if (data.levels) {
				...
				$.each(data.levels, function(index, value) {
					...
					$.each(value.locations, function(index, value) {
						//console.debug('Location', index, value);
						var top = value.y * 100;
						...
						if (value.pin != 'hidden') {
							if (self.o.locations) {
								...
								//							
								var pin = $('<div>' + self.o.pin_label_render(value, iterate_level) + '</div>').addClass('mapplic-pin').css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
								pin.attr('data-location', value.id);
								pin.addClass(value.pin);
								
								$(pin).drag(function(ev, dd) {
									var x = (ev.pageX - self.map.offset().left) / self.map.width(), y = (ev.pageY - self.map.offset().top) / self.map.height();
									...
									$( this ).css({
										top: toppct + '%', //dd.offsetY,
										left: leftpct + '%' //dd.offsetX
									});
								},{ relative:true }).drag('end', function(ev, el) {
									var x = (ev.pageX - self.map.offset().left) / self.map.width(), y = (ev.pageY - self.map.offset().top) / self.map.height();
									self.o.location_changed(value.id, x, y, el);
								});
			...
			if (self.o.initial_level) {				
				var initial_level = self.o.initial_level();
				level(self._levels.hasOwnProperty(initial_level) ? initial_level : shownLevel);
			} else {
				level(shownLevel);
			}
		...
		var level = function(target) {
			...
			self.o.level_changed(target, self._levels[target]);
